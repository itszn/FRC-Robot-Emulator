...
public class NetworkTable implements ITable, IRemote {
	...
	public synchronized static NetworkTable getTable(String key) {
		...
		return new NetworkTable("",null);//(NetworkTable)staticProvider.getTable(PATH_SEPARATOR+key);
	}
	...
	private final NetworkTableNode node = null;
	...
	public Object getValue(String key) throws TableKeyNotDefinedException {
		return table.get(key);//node.getValue(absoluteKeyCache.get(key));
	}
